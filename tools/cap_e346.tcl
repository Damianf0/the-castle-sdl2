...
set ::idx -1
proc dump {rx} { set f [open [format "tests/fixtures/e346/e346_%02X.bin" $rx] w]; fconfigure $f -translation binary; puts -nonewline $f [debug read_block memory 0xe346 64]; close $f }
proc step {} {
    if {$::idx>=0} { dump [debug read memory 0xe320] }
    incr ::idx
    if {$::idx>=[llength $::rooms]} { debug remove_bp $::bp; set f [open e34_done.txt w]; puts $f ok; close $f; exit }
